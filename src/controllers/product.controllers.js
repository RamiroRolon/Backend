...
export const getAll = async (req, res, next) => {
  try {
    ...
    const nextLink = response.hasNextPage ? `http://localhost:${process.env.PORT}/products?page=${response.nextPage}` : null;
    const prevLink = response.hasPrevPage ? `http://localhost:${process.env.PORT}/products?page=${response.prevPage}` : null;
    res.status(200).json({
      status: 'success',
